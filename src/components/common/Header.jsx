...
import {
  Search, Menu, X, LogIn, LogOut, User, Map, BarChart3,
  GitCompareArrows, MessageCircle, Database, Home, School, Settings,
} from 'lucide-react';
...
export default function Header() {
  ...
  // displayName은 useAuth에서 profiles DB 기준으로 제공
  const userName = displayName || '사용자';
      ...
      <div className="max-w-7xl mx-auto px-4 lg:px-6">
        <div className="flex items-center justify-between h-16">
          <Link to="/" className="flex items-center gap-3 shrink-0">
            ...
            <div>
              <h1 className="text-white font-bold text-base tracking-tight leading-tight">세종 스쿨이음톡</h1>
              <p className="text-navy-300 text-[10px] font-normal">SejongSchoolEumTalk</p>
            </div>
          </Link>
            ...
            {navItems.map((item) => {
              ...
              return (
                <NavLink key={item.path} to={item.path} end={item.path === '/'}
                  ...
                  {item.staffOnly && (
                    <span className="text-[9px] bg-primary-500/30 text-primary-300 px-1.5 py-0.5 rounded-full">내부</span>
                  )}
                </NavLink>
              );
            })}
          ...
          <div className="flex items-center gap-2">
            {searchOpen ? (
              <form onSubmit={handleSearch} className="relative">
                <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
                  placeholder="학교명 또는 질문 입력..." autoFocus
                  className="w-48 md:w-64 bg-white/15 border border-white/20 text-white rounded-full px-4 py-1.5 pl-9 text-sm placeholder-white/40 focus:outline-none focus:bg-white/20 focus:border-primary-400" />
                <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-white/50" />
                <button type="button" onClick={() => setSearchOpen(false)}
                  className="absolute right-2 top-1/2 -translate-y-1/2 text-white/50 hover:text-white"><X size={14} /></button>
              </form>
            ) : (
              <button onClick={() => setSearchOpen(true)} className="p-2 text-white/60 hover:text-white transition-colors">
                <Search size={18} />
              </button>
            )}
            ...
            {user ? (
              <div className="hidden sm:flex items-center gap-3 ml-2">
                <Link to="/profile" className="flex items-center gap-1.5 text-white hover:text-primary-300 transition-colors group" title="개인정보 관리">
                  ...
                  <span className="text-sm font-medium">
                    {userName}<span className="text-white/50 font-normal ml-0.5">님</span>
                  </span>
                </Link>
                <button onClick={signOut}
                  className="flex items-center gap-1.5 px-3 py-1.5 text-xs text-white/70 hover:text-white border border-white/20 rounded-full hover:bg-white/10 transition-all">
                  <LogOut size={13} /><span>로그아웃</span>
                </button>
              </div>
            ) : (
              <Link to="/login"
                className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 text-xs text-white bg-primary-500 rounded-full hover:bg-primary-600 transition-all shadow-sm">
                <LogIn size={13} /><span>로그인</span>
              </Link>
            )}
              ...
              {user ? (
                <div className="flex flex-col gap-1">
                  <Link to="/profile" onClick={() => setMobileOpen(false)}
                    ...
                    <User size={16} /><span className="text-sm font-medium">{userName}님</span>
